...
function DepositView({ onBack }: { onBack: () => void }) {
  ...
  // Gera QR code no frontend quando o MercadoPago nÃ£o retornar um
  const [localQr, setLocalQr] = useState<string | null>(null);
  useEffect(() => {
    if (!pixCode || pendingDeposit?.pixQrCode) { setLocalQr(null); return; }
    import("qrcode").then(QRCode => {
      QRCode.toDataURL(pixCode, { width: 256, margin: 1 }).then(setLocalQr).catch(() => setLocalQr(null));
    });
  }, [pixCode, pendingDeposit?.pixQrCode]);
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-xl p-4 space-y-3 border border-blue-100 bg-white/80">
          ...
          {/* QR Code â€” MP dinÃ¢mico ou gerado localmente */}
          {(pendingDeposit.pixQrCode || localQr) && (
            <div className="flex flex-col items-center gap-2">
              <img
                src={pendingDeposit.pixQrCode ? `data:image/png;base64,${pendingDeposit.pixQrCode}` : localQr!}
                alt="QR Code PIX"
                className="w-48 h-48 rounded-lg border border-zinc-200 bg-white p-2"
              />
        ...
        <p className="text-xs text-gray-500 text-center">ApÃ³s enviar o PIX, seu saldo serÃ¡ atualizado em atÃ© 30 min.</p>
      </div>
    );
  }
